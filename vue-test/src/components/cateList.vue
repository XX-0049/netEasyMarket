  ...
  <div style="position: relative;padding-left:2.16rem;">
    ...
    <div class="sidebarClass">
      <van-sidebar v-model="sidebarKey" @change="onChangeSidebar">
        <van-sidebar-item v-for="(n, inx) in sidebar_arr" :title="n.txt" :key="inx" />
      </van-sidebar>
    </div>

    <!-- 右侧商品区 -->
    <div class="sidebarGoods">
      <div class="goodsItem" v-for="(n, inx) in goodsListObj" @click="gotoGoodsBuy(n)">
        <img :src="n.goods_img" />
        <p>{{n.goods_name}}</p>
        <b>{{n.goods_price}}</b>
      </div>
...
export default {
  ...
  created() {
    axios.get("http://localhost:3344/cate_goods_list_0").then((_d) => {
      // console.log( _d.data );
  ...
  methods: {
    ...
    onChangeSidebar(_inx) {
      // console.log( _inx );
      axios.get("http://localhost:3344/cate_goods_list_" + _inx).then((_d) => {
        // console.log( _d.data );
        this.goodsListObj = _d.data;
      });
    },
